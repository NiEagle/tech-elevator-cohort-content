...
public class Exercise07_StoreHours {
    ...
    public boolean isStoreOpen(int currentHour, char currentDay, boolean isSummer) {
        if (!isSummer) {
            if (currentHour < 17 && currentHour >= 8 && (currentDay == 'M' || currentDay == 'W' || currentDay == 'F')) {
                return true;
            }
        }

        //SPECIAL CASES
        else if ((currentDay == 'M' || currentDay == 'F') && currentHour < 17 && currentHour >= 8){
            return true;
        }
        else if (currentDay == 'W' && currentHour < 20 && currentHour >= 8){
            return true;
        }
        else if (currentDay == 'S' && currentHour < 15 && currentHour >= 9){
            return true;
        }
